...
void read_syn_bfee(unsigned char *inBytes, mxArray *outCell)
{
	unsigned int timestamp_high = ((inBytes[4]) + (inBytes[5] << 8) + (inBytes[6] << 16) + (inBytes[7] <<24));
	unsigned int timestamp_low = inBytes[0] + (inBytes[1] << 8) +
		(inBytes[2] << 16) + (inBytes[3] << 24);
	unsigned short bfee_count = inBytes[8] + (inBytes[9] << 8);
	unsigned int Nrx = inBytes[12];
	unsigned int Ntx = inBytes[13];
	unsigned int rssi_a = inBytes[14];
	unsigned int rssi_b = inBytes[15];
	unsigned int rssi_c = inBytes[16];
	char noise = inBytes[17];
	unsigned int agc = inBytes[18];
	unsigned int antenna_sel = inBytes[19];
	unsigned int len = inBytes[20] + (inBytes[21] << 8);
	unsigned int fake_rate_n_flags = inBytes[22] + (inBytes[23] << 8);
	...
	if (len != calc_len)
		mexErrMsgIdAndTxt("MIMOToolbox:read_syn_bfee_new:size","Wrong beamforming matrix size.");
	...
	mxDestroyArray(mxGetField(outCell, 0, "timestamp_high"));
	...
	mxSetField(outCell, 0, "timestamp_high", mxCreateDoubleScalar((double)timestamp_high));
...
{
	const char* fieldnames[] = {"timestamp_high",
		"timestamp_low",
		"bfee_count",
		"Nrx", "Ntx",
	...
	if(nrhs!=1) {
		mexErrMsgIdAndTxt("MIMOToolbox:read_syn_bfee_new:nrhs","One input required.");
	}
	if(nlhs!=1) {
		mexErrMsgIdAndTxt("MIMOToolbox:read_syn_bfee_new:nlhs","One output required.");
	}
	/* make sure the input argument is a char array */
	if (!mxIsClass(prhs[0], "uint8")) {
		mexErrMsgIdAndTxt("MIMOToolbox:read_syn_bfee_new:notBytes","Input must be a char array");
	}
